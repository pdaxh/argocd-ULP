...
if [ ! -f "Chart.yaml" ]; then
    echo "ERROR: Error: Please run this script from the argocd-ULP directory"
    exit 1
fi
...
check_external_secrets() {
    if kubectl get crd externalsecrets.external-secrets.io >/dev/null 2>&1; then
        echo "SUCCESS: External Secrets Operator detected"
        return 0
    else
        echo "ERROR: External Secrets Operator not found"
        return 1
    fi
}
...
check_sealed_secrets() {
    if kubectl get crd sealedsecrets.bitnami.com >/dev/null 2>&1; then
        echo "SUCCESS: Sealed Secrets detected"
        return 0
    else
        echo "ERROR: Sealed Secrets not found"
        return 1
    fi
}
...
get_credentials() {
    ...
    if [ -z "$GITHUB_USERNAME" ] || [ -z "$GITHUB_TOKEN" ]; then
        echo "ERROR: Error: Username and token cannot be empty"
        exit 1
    fi
    ...
    if curl -s -H "Authorization: token $GITHUB_TOKEN" https://api.github.com/user | grep -q "login"; then
        echo "SUCCESS: GitHub credentials validated successfully"
    else
        echo "ERROR: Error: Invalid GitHub credentials"
        exit 1
    fi
}
...
deploy_with_external_secrets() {
    ...
    echo "WARNING:  Note: You need to configure your external secret store (Vault, AWS Secrets Manager, etc.)"
...
EOF
    ...
    echo "SUCCESS: Secret sealed and deployed successfully"
}
...
deploy_with_env_vars() {
    ...
    echo "SUCCESS: Secret deployed using environment variables"
}
...
main() {
    ...
    if check_external_secrets; then
        ...
        echo "WARNING:  No secure secret management detected"
        echo " Available options:"
    ...
    echo "SUCCESS: Secure deployment complete!"
    echo ""
